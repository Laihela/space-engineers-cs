...
void Main() {
	Base.Title("Transmitter");
	MyDetectedEntityInfo target = Sensors.Detect();
	if (target.TimeStamp == 0) Base.Print("Scanning");
	else {
		Base.Print(target.Name);
		Base.Print("Range " + ((target.Position - Me.GetPosition()).Length() / 1e3).ToString("0.00") + " km");
		Base.Print("Speed " + (target.Velocity.Length()).ToString("0.00") + " m/s");
		Base.Print("X " + target.Position.X.ToString("0.0"));
		Base.Print("Y " + target.Position.Y.ToString("0.0"));
		Base.Print("Z " + target.Position.Z.ToString("0.0"));
		Comms.Transmit("Target", new TargetTracking.Target(target).Serialize());
	}
}
