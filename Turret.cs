Program () {
	...
	TargetTracking.MissingTargetExpireTime = 1.0;
	RotorTurret.MaxDeviation = 3.0;
}
...
void Main() {
	...
	if (TargetTracking.CurrentTarget == null) {
		...
		Base.Print("Engaging");
		Base.Print("Last seen " + (DateTime.Now - TargetTracking.CurrentTarget.LastDetected).TotalSeconds.ToString("0.00") + " s");
		...
		RotorTurret.Target(TargetTracking.GetVectors(), true);
	}
}

static class TEMPALTE {
	public static readonly DateTime Version = new DateTime(2020, 10, 14, 19, 56, 00);
...
static class Rotors {
	...
	public static double AimCorrectionRate = 128.0;
	
	// Remember to call this first
	public static void Init(MyGridProgram program) {
		Program = program;
		Program.Echo("Initializing Rotors");
		Program.Echo("  Version: " + Version.ToString("yy.MM.dd.HH.mm"));
		RotorBlocks = Base.GroupBlocks.OfType<IMyMotorStator>().ToList();
		Program.Echo("    RotorBlocks: " + RotorBlocks.Count);
	}
	
	// Aim rotors towards a position, an angle or rotate them at velocity.
	public static void Target(double? angle = null, Vector3D? position = null, double? velocity = null) {
		foreach (IMyMotorStator rotor in RotorBlocks) {
			double deviation = -rotor.Angle;
			if (position != null) {
				Vector3D localTarget = Util.VectorToBlockSpace((Vector3D)position, rotor);
				deviation += Math.Atan2(-localTarget.X, localTarget.Z);
				if (rotor.CustomData != "") deviation += double.Parse(rotor.CustomData) / 180.0 * Math.PI;
			}
			if (angle != null) deviation += (double)angle;
			if (deviation < -Math.PI) deviation += Math.PI * 2;
			double targetVelocity = velocity != null ? (double)velocity : 0.0;
			rotor.SetValue<float>("Velocity",  (float)(deviation * AimCorrectionRate + targetVelocity));
		}
	}
}
